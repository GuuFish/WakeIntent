...
WakeIntent 是一个面向对话式 AI、与具体框架无关的联系意图引擎。它把对话中“未来值得再次联系用户的理由”转化为可持久化的 `ContactIntent`，并在真正联系前结合最新上下文重新验证，最终决定联系、推迟、取消、过期、完成或保持沉默。
...
使用同一个状态文件再次运行相同命令，可以观察幂等重启行为：旧意图不会被重复联系或重复评估。
...
这两项测试使用合成对话，但会调用你配置的真实模型，并把完整、可审计的报告写入 `reports/core-api-smoke/`。
...
这些包目前仍是私有工作区包，尚未发布到 npm。在公共 API 稳定前，可以直接从此 monorepo 嵌入使用。

## 当前证据

截至 2026 年 9 月 3 日，完整仓库检查通过，五个包共 178 项测试全部通过。两次使用 `gpt-5.5` 的全新真实模型冒烟测试也已通过：
